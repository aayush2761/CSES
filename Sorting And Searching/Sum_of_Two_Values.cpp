...
// aur sir shortcut ke chakkar mein *o** mt lgwao wrna.....!!
...
int32_t main() {
    ...
    auto solve = [&]() {
        ...
        int i = 0, j = n - 1;
        while (i < j) {
            int sum = indexed_v[i].first + indexed_v[j].first;
            if (sum == x) {
                // Print the original indices
                cout << indexed_v[i].second << " " << indexed_v[j].second << endl;
                return;
            } else if (sum > x) {
                j--;
            } else {
