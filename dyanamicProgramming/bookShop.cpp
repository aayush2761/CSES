...
int bookShop(int target,int n , vi&price, vi&pages){
     // dp[i] represents the maximum number of pages with a total cost of i.
    vi dp(target + 1, 0);
    for (int j = 0; j < n; j++) { 
        for (int i = target; i >= price[j]; i--) { 
            dp[i] = max(dp[i]%M, (dp[i - price[j]] + pages[j])%M);
        }
    }
    return dp[target];
...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        int res=bookShop(x,n , price,pages);
        out(res)
    };
