...
int coinCombinationTwo(vi&coins, int target){
    // dp[i][target]->number of ways to get sum x using coins i to n and all coins are skipped before i
...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        vi coins(n);
        // sort(all(coins));
        foreach(i,0,n,1)cin>>coins[i];
        int res= coinCombinationTwo(coins,target);
        out(res)
    };
